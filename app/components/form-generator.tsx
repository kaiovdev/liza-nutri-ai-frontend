...
export function FormGenerator({ data }: { data: FormProps }) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4">
      <TextType
        text={["Ótimo, agora com seus dados podemos gerar a sua dieta", "Aperte o botão abaixo e gere a sua dieta!"]}
        ...
        {output && (
          <div className="bg-card rounded-lg p-6 border border-border max-h-[500px] overflow-y-auto">
            <div className="prose prose-sm max-w-none">
              <ReactMarkdown
                components={{
                  h2: ({ node, ...props }) => <h2 className="text-xl font-bold text-green-600 my-1" {...props} />,
                  h3: ({ node, ...props }) => <h3 className="text-xl font-bold text-green-600 my-1" {...props} />,
                  h1: ({ node, ...props }) => <h1 className="text-2xl font-bold text-zinc-900 mb-1" {...props} />,
                }}
              >
                {output}
              </ReactMarkdown>
